...
4. **No accidental zoom** — The tools are fixed-layout and tap-heavy, so page zoom is disabled three ways, all of which a new tool must copy: `touch-action: manipulation` on the `*` rule, `maximum-scale=1, user-scalable=no` in the viewport meta, and the double-tap guard script before `</body>` (iOS ignores the first two, so the script is what actually stops it there).
