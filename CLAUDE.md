...
4. **No accidental zoom** — The tools are fixed-layout and tap-heavy, so page zoom is disabled entirely and a new tool must copy all three layers: `touch-action: pan-x pan-y` on the `*` rule (scrolling stays, double-tap and pinch zoom go), `maximum-scale=1, user-scalable=no` in the viewport meta, and the guard script before `</body>` that cancels double-taps, two-finger moves and Safari's `gesture*` events. iOS ignores the first two, so the script is what actually stops it there.
